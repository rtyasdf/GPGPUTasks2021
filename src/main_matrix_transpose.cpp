...
int main(int argc, char **argv)
{
    ...
    
    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            ...
            while (global_work_size_x % work_group_size_x > 0)
                global_work_size_x++;

            unsigned int work_group_size_y = 16;
            unsigned int global_work_size_y = M;
            while (global_work_size_y % work_group_size_y > 0)
                global_work_size_y++;
            ...
            matrix_transpose_kernel.exec(gpu::WorkSize(work_group_size_x, work_group_size_y, global_work_size_x, global_work_size_y), as_gpu, as_t_gpu, K, M);
            t.nextLap();
        }
